...
const pagesDir = path.resolve(srcRoot, 'pages');
const type = process.argv[3];
const devPath = path.resolve(__dirname, 'public'); //开发目录
const distPath = path.resolve(__dirname, './dist'); //打包目录
const mainFile = 'main.js';
// const fielConfig = ['page1', 'page2']; //打包规则
const isDev = type === 'webpack.config.dev.js' ? true : false; //判断环境

function getHtmlArray(entryMap) {
  const htmlArray = [];
  Object.keys(entryMap).forEach(key => {
    let fullPathName = path.resolve(pagesDir, key);
